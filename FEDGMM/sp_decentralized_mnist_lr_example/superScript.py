...
def run_federation_with_semaphore(semaphore, lr, scenario, gpu_id):
    run_name = f"Scenario:{scenario}_lr:{lr}_FedGMM"
    try:
        run_federation(lr, scenario, gpu_id, run_name)
    finally:
...
def run_federation(lr, scenario, gpu_id, run_name):
...
def update_and_run_config(gpu_id):
    ...
    for lr in learning_rates:
        for scenario in scenarios:                              
            ...
            #     time.sleep(120)
            p = Process(target=run_federation_with_semaphore, args=(semaphore, lr, scenario, gpu_id))
            processes.append(p)
            p.start()
            time.sleep(10)
