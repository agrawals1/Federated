...
batch_sizes = [64]
strategies = ["all_participate", "_client_sampling", "client_sampling_cyclic_overlap_pattern"]
learning_rates = [0.1] 
decays = [1e-5]
# betas = [0.05, 0.1, 0.5, 10.0]  # parameter for dirichlet distribution
betas = [0.1, 0.5, 1.0, 100.0]
...
def run_federation(beta, overlap, gpu_id, round, lr, bs, decay, seed, group_norm, freq, decay_fact, epochs, run_name, strategy, dataset):
                                        ...
                                        for beta in betas:
                                            for overlap in overlaps:
                                                for seed in seeds:                                
                                                    ...
                                                    p = Process(target=run_federation_with_semaphore, args=(semaphore, beta, overlap, gpu_id, round, lr, bs, decay, seed, group_norm, freq, decay_fact, strategy, dataset))
                                                    processes.append(p)
                                                    p.start()
                                                    time.sleep(200)
